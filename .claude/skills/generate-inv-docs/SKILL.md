---
name: generate-inv-docs
description: "Generate markdown documentation for each record in a selected .inv file. Saves files to documentation/ folder. Usage: /generate-inv-docs"
---
...
Ask the user which `.inv` file to process. If a file path was provided as argument, use that. Otherwise list `.inv` files in the current directory and `data/` folder:
...
Create the documentation output folder:
```bash
